...
/**
 * Eigen kopie van het gzip-interceptorpatroon uit `placesearch`/`personsearch`: deze module heeft
 * `allowedDependencies = {}` en mag dus niet op die modules steunen. Vraagt `Accept-Encoding: gzip`
 * op en zet een beschrijvende User-Agent-header; decomprimeert transparant een gzip-gecodeerde
 * respons.
 */
class TopicSearchGzipRequestInterceptor(private val userAgent: String) : ClientHttpRequestInterceptor {
